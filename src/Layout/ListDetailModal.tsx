...
// Describes the values this popi recieves
...
export const ListDetailModal = ({ open, list, onClose, onEdit }: ListDetailModalProps) => {
  // Hide the popup when closed/no list is selected
  if (!open || !list) return null

    return (
          <>
            {/* Cover the screen and center the popup */}
            <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4'>
             {/* Allow scrolling when the content is too long */}
             <div className='max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl bg-white p-6 shadow-lg'>
                {/* show the list name and close icon. */}
            ...
            {list.items.map((item) => (
             <div key={item.id} className='flex items-center gap-3 py-3'>
               {/* Shows the item image/an empty gray box */}
               {item.image ? (
               ...
               {/* checks out the name when the item is checked. */}
                <div className='flex-1'>
                 <p className={`text-sm ${item.checked ? 'text-gray-400 line-through' : 'text-gray-900'}`}>
                  {item.name}
                  </p>
                 </div>
                 {/* shows how many of this items are needed. */}
               <span className='text-sm text-gray-500'>x{item.quantity}</span>
             </div>
             ))}
            </div>
             
             {/* let the user close the popup/edit the list. */}
